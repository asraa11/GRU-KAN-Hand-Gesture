...
class Config:
    ...
    MODEL_SAVE_PATH = "hand_gesture_gru_kan_att2dir360.pth"  # لم يتغير


# ==================== FIXED MODEL: GRU then KAN (NO LINEAR, KEEPS 32) ====================
class HandGestureGRUKAN(nn.Module):
    """Correct architecture: GRU first, then KAN - NO LINEAR LAYER, KEEPS 32"""

    def __init__(self, time_steps=360, input_features=14, num_classes=7):
        super().__init__()

        print("\n" + "=" * 60)
        print("🧠 BUILDING CORRECT MODEL: GRU → KAN (NO LINEAR, KEEPS 32)")
        print("=" * 60)
        ...
        self.bn = nn.BatchNorm1d(self.gru_output_size)  # ⬅️ FIXED: 256
...
def train_hand_gesture_model():
    ...
    plt.title(f'Confusion Matrix - HandGestureGRUKAN Model (No Linear, with 32)')
...
def main():
    print("=" * 70)
    print("🤖 HAND GESTURE RECOGNITION - CORRECT ARCHITECTURE (NO LINEAR, WITH 32)")
    print("=" * 70)
    print("\n📋 Correct Flow: GRU (temporal) → KAN (logits)")
    print(f"📁 Data folder: '{Config.DATA_FOLDER}'")
    print(f"📊 KAN width: [{Config.GRU_HIDDEN_SIZE*2}, 64, 32, {Config.CLASSES}]")

    print("\n🎯 Available Options:")
    print("   1. Train model (GRU → KAN, no Linear, with 32)")
    ...
    if choice == '1':
        print("\n" + "=" * 60)
        print("🚀 TRAINING MODEL: GRU → KAN (NO LINEAR, WITH 32)")
        print("=" * 60)
