...
READS ONLY ODD FRAMES (1, 3, 5, ..., 359)
...
# ==================== CONFIGURATION ====================
class Config:
    TIME_POINTS = 180  # Changed from 360 to 180 (only odd frames)
    FEATURES = 14
    CLASSES = 7
    SAMPLING_RATE = 90
    ...
    MODEL_SAVE_PATH = "hand_gesture_gru_kan_odd_frames.pth"


# ==================== SIMPLE MODEL: GRU then KAN ====================
class HandGestureGRUKAN(nn.Module):
    """Simple version without attention - KAN outputs logits directly (NO Linear)"""

    def __init__(self, input_features=14, num_classes=7):
        super().__init__()

        print("\n" + "=" * 60)
        print("🧠 BUILDING HAND GESTURE RECOGNITION MODEL")
        ...
        # KAN for classification - width ends with num_classes (NO Linear)
        print(f"   • KAN Layer: {self.gru_output_size} → 64 → 32 → {num_classes}")
        self.kan = KAN(
            width=[self.gru_output_size, 64, 32, num_classes],  # ✅ آخر طبقة = num_classes
        ...
        print(f"   Architecture: Input → LayerNorm → GRU → KAN → Output (logits)")
...
class HandGestureDataProcessor:
    def __init__(self, target_length=180):  # Changed to 180 for odd frames only
        self.target_length = target_length  # 180 frames (odd frames only)
    ...
    def load_single_file(self, file_path: str) -> Tuple[np.ndarray, str]:
        try:
            ...
            # Take only ODD frames (1, 3, 5, ..., 359)
            # Using iloc to access rows by index: [0, 2, 4, ...]
            # This takes first row (frame 1), third row (frame 3), etc.
            if len(df) >= 2:  # If we have at least 2 frames
                # Take every second row starting from first row (index 0)
                df_odd_frames = df.iloc[::2].reset_index(drop=True)
                feature_data = df_odd_frames[self.required_features].values.astype(np.float32)
            else:
                # If file is too short, take all data
    ...
    def load_all_files(self, data_folder: str) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        print(f"📊 Note: Loading only ODD frames (1, 3, 5, ..., 359) from each file")
        ...
        print(f"   Note: Using {self.target_length} time points (odd frames only)")
...
def plot_feature_importance(model, feature_names, num_samples=10):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            ax.set_title('Feature Importance based on GRU Activations')
...
def train_hand_gesture_model():
    ...
    print(f"   Architecture: Input({Config.FEATURES}) → LayerNorm → GRU → KAN → Output({Config.CLASSES})")
    print(f"   Time points: {Config.TIME_POINTS} (odd frames only)")
    print(f"   ✅ KAN outputs {Config.CLASSES} logits directly (NO Linear)")
    ...
    plt.title('Confusion Matrix - Hand Gesture Recognition Model (Odd Frames Only)')
    ...
    print(f"✅ Model uses KAN only (NO Linear classifier)")
...
def main():
    ...
    print("=" * 70)
    print("\n📋 Model Architecture: GRU (temporal) → KAN (non-linear)")
    ...
    if choice == '1':
        print("\n" + "=" * 60)
        print("🚀 TRAINING MODEL (ODD FRAMES ONLY)")
        print("=" * 60)
        ...
        print("\n" + "=" * 60)
        print("🧪 TESTING SINGLE FILE (ODD FRAMES ONLY)")
        print("=" * 60)
        ...
        try:
            ...
            processor = HandGestureDataProcessor(target_length=checkpoint['config']['time_steps'])
            ...
            if predicted:
                ...
                print(f"   ✅ KAN only (NO Linear classifier)")
                print(f"\n⏱️  Timing Information:")
                print(f"   • Prediction time: {prediction_time*1000:.2f} ms")
                print(f"   • Real-time factor: {prediction_time/(Config.TIME_POINTS/(Config.SAMPLING_RATE/2)):.3f}x")
