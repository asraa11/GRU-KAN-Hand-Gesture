"""
HAND GESTURE RECOGNITION - CORRECTED VERSION (Fixed BatchNorm)
GRU first, then KAN (NO LINEAR LAYER) -
UNI-DIRECTIONAL GRU VERSION
"""
...
class Config:
    ...
    MODEL_SAVE_PATH = "hand_gesture_gru_kan_unidirectional_no_linear.pth"


# ==================== FIXED MODEL: GRU then KAN (NO LINEAR, KEEP 32) ====================
class HandGestureGRUKAN(nn.Module):
    """Correct architecture: UNI-DIRECTIONAL GRU → KAN (no Linear layer, keeps 32 neurons)"""

    def __init__(self, time_steps=360, input_features=14, num_classes=7):
        super().__init__()

        print("\n" + "=" * 60)
        print("🧠 BUILDING CORRECT MODEL: UNI-DIRECTIONAL GRU → KAN (NO LINEAR)")
        ...
        # ✅ تم حذف self.classifier بالكامل
    ...
    def forward(self, x):
        ...
        # KAN outputs logits directly (NO Linear)
        logits = self.kan(context)  # [batch, num_classes]
        return logits
...
def plot_training_history(train_losses, val_losses, train_accuracies, val_accuracies):
    ...
    plt.tight_layout()
    plt.savefig('training_history_unidirectional_no_linear.png', dpi=100, bbox_inches='tight')
    plt.show()
...
def plot_feature_importance(model, feature_names, num_samples=10):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            ax.set_title('Feature Importance based on UNI-DIRECTIONAL GRU Activations')
            ax.grid(True, alpha=0.3, axis='x')

            plt.tight_layout()
            plt.savefig('feature_importance_unidirectional_no_linear.png', dpi=100, bbox_inches='tight')
            plt.show()
...
def train_hand_gesture_model():
    ...
    print(f"   • Frames per sample: {Config.TIME_POINTS} frames")
    ...
    plt.title(f'Confusion Matrix - UNI-DIRECTIONAL HandGestureGRUKAN (No Linear)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.tight_layout()
    plt.savefig('confusion_matrix_unidirectional_no_linear.png', dpi=100, bbox_inches='tight')
    plt.show()
...
def main():
    print("=" * 70)
    print("🤖 HAND GESTURE RECOGNITION - UNI-DIRECTIONAL GRU (NO LINEAR LAYER)")
    print("=" * 70)
    print("\n📋 Correct Flow: UNI-DIRECTIONAL GRU (temporal) → KAN (output logits)")
    print(f"📁 Data folder: '{Config.DATA_FOLDER}'")
    print(f"📊 KAN width: [{Config.GRU_HIDDEN_SIZE}, 64, 32, {Config.CLASSES}]")

    print("\n🎯 Available Options:")
    print("   1. Train model (UNI-DIRECTIONAL GRU → KAN, no Linear)")
    ...
    if choice == '1':
        print("\n" + "=" * 60)
        print("🚀 TRAINING MODEL: UNI-DIRECTIONAL GRU → KAN (NO LINEAR)")
        print("=" * 60)
        ...
        if model is not None:
            ...
            print(f"   • training_history_unidirectional_no_linear.png - Training plots")
            print(f"   • confusion_matrix_unidirectional_no_linear.png - Confusion matrix")
